...
def evaluate_policy(
    ...
    for ep in range(BATCH_SIZE//num_steps):
        # print(f"[Evaluation] Starting episode {ep+1}...")
        ...
        if SAVE_GIF and ep==0:
            if current_step=="final":
                print("Saving initial frame for final evaluation GIF...")
                img = env.render(env_state)
                frames.append(np.asarray(img))
            elif current_step % SAVE_GIF_INTERVAL == 0:
                print("Saving initial frame for evaluation GIF at step {}...".format(current_step))
                img = env.render(env_state)
                frames.append(np.asarray(img))
        ...
        for t in range(num_steps):
            ...
            if SAVE_GIF and ep==0:
                if current_step=="final":
                    # print("Saving final evaluation GIF...")
                    img = env.render(env_state)
                    frames.append(np.asarray(img))
                elif current_step % SAVE_GIF_INTERVAL == 0:
                    img = env.render(env_state)
    ...
    if log_wandb:
        wandb.log({
            ...
            "eval/episode_return_mean": jnp.nanmean(episode_return),
            # "eval/episode_return_total": episode_return.sum().item(),
            "eval/episode_return_total": jnp.nansum(episode_return),
